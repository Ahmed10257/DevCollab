...
@Query('assignedUserId', new ParseIntPipe({ optional: true }))
assignedUserId?: number,
@Query('responsibleUserId', new ParseIntPipe({ optional: true }))
...
findServers() {
return this.assetService.findAll({ categoryId: 7 });
}

@Get('category/racks')
findRacks() {
return this.assetService.findAll({ categoryId: 8 });
}

@Get('category/printers')
findPrinters() {
return this.assetService.findAll({ categoryId: 9 });
}

@Get('category/projectors')
findProjectors() {
return this.assetService.findAll({ categoryId: 10 });
}

@Get('category/cameras')
findCameras() {
return this.assetService.findAll({ categoryId: 11 });
}

@Get('category/ip-phones')
findIpPhones() {
return this.assetService.findAll({ categoryId: 12 });
}
