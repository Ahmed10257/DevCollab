pipeline {
  ...
  stages {
    ...
    stage('Build ContainerD Images') {
      steps {
        sh "nerdctl build --namespace k8s.io --address /run/containerd/containerd.sock -t devcollab-frontend:latest ./front-end"
        sh "nerdctl build --namespace k8s.io --address /run/containerd/containerd.sock -t devcollab-backend:latest ./back-end"
        }
    }
